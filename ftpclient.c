...
#define	TRUE	1
#define	FALSE	0
void trim(char *str) {		//function trims leading and trailing whitespaces
	int i;
    int start = 0;
    ...
    while ((stop >= start) && isspace((unsigned char) str[stop])) {
    	stop--;
    }
    for (i = start; i <= stop; i++) {	// Shift all characters back to the start of the string array.
    	str[i - start] = str[i];
    }
    str[i - start] = '\0'; // Null terminate string.
}
int user_input(char *buffer){	//clear buffer
    memset(buffer, 0, (int)sizeof(buffer));
    printf("> ");		 //print prompt
    if(fgets(buffer, 1024, stdin) == NULL){		//get user input
            return -1;
    }
    return 1;
}
...
int main(int argc, char **argv){
	...
	sscanf(argv[2], "%d", &server_port);	//get server port
    ...
    get_ip_port(controlfd, ip, (int *)&x);		//get ip address from control port
    //x = 0;
    printf("x: %d\n", x);
    printf("ip: %s\n", ip);
    ...
    while(1){
        bzero(command, strlen(command));
        code = get_command(command);		//get command from user                
        if(code == 4){			//user has entered quit
            char quit[1024];
            sprintf(quit, "QUIT");
